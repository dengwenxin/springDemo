            ...
            fillCalData: function(indexDay, monthFirstD, monthLastD, nowDay) {
                ...
                for (var i = 0; i < rows; i++) {
                    ...
                    for (var j = 0; j < 7; j++) {
                        ...
                        // 临时调整节气
                        // var date_detail =
                        // aDay.value.getFullYear()+"-"+(aDay.value.getMonth()+1)+"-"+aDay.value.getDate();
                        week.push(aDay);
                ...
                for (var i = 0; i < rows; i++) {
                    ...
                    for (var j = 0; j < 7; j++) {
                        if (calData[i][j] != null) {
                            ...
                            if (is5Row)
                                aDay =
                                        "<td i=" + i + " j=" + j + " style='height:" + (1 / rows).toPercent()
                                                + ";' class='block block5 " + tdclass + isClickBlock + "'>";
                            else
                                aDay =
                                        "<td i=" + i + " j=" + j + " style='height:" + (1 / rows).toPercent()
                                                + ";' class='block " + tdclass + isClickBlock + "'>";

                            aDay +=
                                    "<a class='block_content'" + "data='" + (calData[i][j].value.format('MMdd'))
                                            + "' href=\"javascript:;\">";
            ...
            makeAction: function() {
                ...
                $('.tdhover').bind(
                        ...
                        function(e) {
                            ...
                            calData[ele.attr('i')][ele.attr('j')].value.setHours(nowTime.getHours(), nowTime
                                    .getMinutes(), nowTime.getSeconds());
...
var templates =
        {
            ...
            lunar_Info_detail: function(date, callback) {
                ...
                info.lunar =
                        '农历' + (cld_day.isLeap ? '闰 ' : '') + templates.getChinaNum(cld_day.lMonth) + "月"
                                + cDay(cld_day.lDay);
                info.y_Info =
                        cld_day.cYear + '年【' + this.lunar_year(date) + "】 " + cld_day.cMonth + '月' + cld_day.cDay + '日';
                try {
